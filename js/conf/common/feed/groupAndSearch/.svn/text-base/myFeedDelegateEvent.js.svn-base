...
STK.register("common.feed.groupAndSearch.myFeedDelegateEvent", function($){
	...
	return function(node, filterObject, opts) {
		...
		function enterKeySubmit () {
			var evt = $.fixEvent();
			evt.target.form.onsubmit = function(){return false;};
			evt = $.getEvent();
		...
		// 搜索定向
		if (isBigPipe) {
			delegate.add('search_rank', 'click', function(spec){
				actions.searchRankChange.call(filterObject, spec);
			});
		}
		delegate.add('search_ranks', 'click', function(spec){
			actions.ranksDisplayToggle.call(filterObject, spec);
		});
