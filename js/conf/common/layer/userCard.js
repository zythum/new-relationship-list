 ...
 * ZhouJiequn @2011.10.18	新增sendMessage引用
 * 
 * 
...
//sendMessage暂时放在这里，虽然会造成冗余，但是方便页面功能调用
$Import('common.dialog.sendMessage');
STK.register('common.layer.userCard', function($){
		/**
		 * 卡片缓存超时时间5分钟
		 */
	...
	return function(node, spec){
		...
		var showCard = function(node, ucData, event){
			 ...
			 * @param {Event} e
			 */
		var hideCard = function(node, ucData, event){
			userCard.stopShow();
			userCard.hideCard();
		};
		...
		var setRemark = function(spec){
			...
			$.common.dialog.setRemark({
				'uid'		: data['uid'],
				'remark'	: remark,
				'callback'	: function (rmk){
					cache.rmCache( data['uid'] );
					cache.rmCache( data['fnick'] );
				}
			});
		};
