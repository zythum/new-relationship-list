...
//					swfAct[pars[key]]&&!STK.core.util.listener.has(conf.channel, pars[key])&&STK.core.util.listener.add(conf.channel,pars[key],swfAct[pars[key]]);
			...
			build: function(){
				...
				//create swf
				$.kit.extra.swfobject.embedSWF(conf.swf_path + conf.ed_swf + '?version=' + conf.version, conf.id_swf, conf.w, conf.h, conf.f_version, conf.swf_path + conf.exp_swf, flashvars, params, attrs);
