...
STK.register('widget.component.success', function($){
	return function(oEntity, oConf){
		...
		it.accept = {
			'success' : function(event, info, json)
			{
				//兼容新旧数据格式
				var html='';
		...
		it.initEvent = function(){
			$.custEvent.add(cId, 'success', it.handler.filter);
		};
		/**
		 * 事件销毁
		 */
		it.destroyEvent = function(){
			$.custEvent.remove(cId, 'success', it.handler.filter);
		};
		return it;
	}
});
