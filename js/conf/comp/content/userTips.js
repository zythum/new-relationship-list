...
STK.register('comp.content.userTips', function($){
	var $L = $.kit.extra.language;
	var $T = $.core.util.templet;
	var TPL = {
		LAYER : '' + 
			'<div node-type="outer" class="layer_tips layer_tips_version layer_tips_invitekey" style="left:100px; top:300px;">' +
				...
				'</div>' +
			'</div>',
		CONTENT : $L('#L{恭喜你！又获得了}') + '#{NUM}' + $L('#L{个邀请码。}')
	};
	...
	return function(node){
		...
		if(!nodes['inviteTips']) return;
		
		var parent = nodes['inviteTips'],
			dialog,
			doms;
