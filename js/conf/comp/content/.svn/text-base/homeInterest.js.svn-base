 ...
 * 可能感兴趣的人
 * 设计说明：
 * dataArr：用于存放从后台获取到的推荐用户
 * loading: 在无用户可显示时，用于标注进入loading状态的标量
 * @author ZhouJiequn | jiequn@staff.sina.com.cn
 * 
 * @history
 * ZhouJiequn @2011.05.05	加关注上行参数fuid改名为uid
 * ZhouJiequn @2011.05.09	修改动画替换效果aniChange函数
 * ZhouJiequn @2011.06.08	可能感兴趣的人改造，增加more区域
 * ZhouJiequn @2011.06.13	可能感兴趣的人改造，增加more区域——bug修改
 */
$Import('common.depand.homeInterest');
...
STK.register('comp.content.homeInterest', function($){
	var MINCOUNT	= 3;
	...
	var	dataArr = [],
		...
		require	= $.common.depand.homeInterest;
	...
	return function(node){
		...
		var ioFuns = {
			...
			getInterest: (function(){
				...
				var myInterest = rAjax.getTrans('mayinterested', {
		...
		//-------DOM事件回调函数定义区-START----------
		var depKey = 'homeInterest_userCard';
